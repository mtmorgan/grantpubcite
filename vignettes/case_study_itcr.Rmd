...
Funded projects are (`contact_pi` is the most recent)
...
projects |>
    ...
    summarize(
        project_start_date = min(project_start_date),
        project_end_date = max(project_end_date),
        years = length(unique(fiscal_year)),
        award_amount = sum(award_amount),
        contact_pi_name = tail(contact_pi_name, 1L),
...
projects |>
    ...
    group_by(foa_tag) |>
    summarize(
        n_projects = length(unique(core_project_num)),
...
plot <-
    project_summaries |>
    ggplot(aes(n_years, amount, text = project_title)) +
    geom_point() +
    geom_smooth(
        method = "lm", formula = y ~ x,
        inherit.aes = FALSE, aes(n_years, amount)
    ) +
    scale_y_continuous(labels = scales::comma) +
...
plot <-
    publications_by_project |>
    mutate(short_title = sprintf("%.40s...", project_title)) |>
    ggplot(aes(amount, n, text = short_title)) +
    geom_point() +
    geom_smooth(
        method = "lm", formula = y ~ x,
        inherit.aes = FALSE, aes(amount, n)
    ) +
...
project_project_citations |>
    ...
    left_join(project_summaries, by = "core_project_num") |>
    select(-cited_by) |>
